...
function mixingIngredients(recipe, ingredients) {
  let combiningMsg = `Combining ingredients for ${ingredients}: `;
  combiningMsg += recipe.map((ingredient) => ingredient.name).join(", ");
  console.log(combiningMsg);
}

function recipeCalculation(recipe) {
  return recipe.reduce((prev, current) => {
    return prev + current.cost;
  }, recipe[0].cost);
}

function recipeIncome(totalCost, profitMargin, pieQuantity) {
...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  recipeIncome(totalCost, profitMargin, pieQuantity);
}
